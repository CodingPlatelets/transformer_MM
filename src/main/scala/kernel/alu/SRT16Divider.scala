...
class SRT16DividerDataModule(len: Int) extends Module {
  val io = IO(new Bundle() {
    ...
    val sign, kill_w, kill_r, isHi, isW = Input(Bool())
...
//  val lzcDiff = Mux(state(s_pre_0), lzcWireDiff, lzcRegDiff)
  ...
  // s_pre_1
  val quotSign =
    Mux(state(s_idle), aSign ^ dSign, true.B) // if not s_idle then must be s_pre_1 & dIsZero, and that we have
  val rSign = aSign
  ...
  mNeg := VecInit(
    Cat(
      SignExt(MuxLookup(dNormReg(len - 2, len - 4), "b00000000".U(7.W))(mLookUpTable2.minus_m(0)), 11),
      0.U(1.W)
    ), // (2, 5) -> (6, 6)
    Cat(
      SignExt(MuxLookup(dNormReg(len - 2, len - 4), "b00000000".U(7.W))(mLookUpTable2.minus_m(1)), 10),
      0.U(2.W)
    ), // (3, 4) -> (6, 6)
  ...
  rudPmNeg := VecInit(Seq.tabulate(5) { i =>
    VecInit(Seq.tabulate(4) { j => SignExt(rudNeg(i)(9, 1), 10) + mNeg(j)(10, 1) })
  })
  r2udPmNeg := VecInit(Seq.tabulate(5) { i =>
    VecInit(Seq.tabulate(4) { j => SignExt(r2udNeg(i), 13) + SignExt(mNeg(j), 13) })
  })
...
//  val rudNegReg = RegEnable(rudNeg, state(s_pre_1))
  ...
  rCarryIter := Mux(
    ~oddIter & finalIter,
    (csaWide1.io.out(1) << 1)(itn_len - 1, 0),
    (csaWide2.io.out(1) << 1)(itn_len - 1, 0)
  )
  ...
  quotIter := Mux(
    state(s_iter),
    quotIterNext,
    Mux(state(s_pre_1), 0.U(len.W), Mux(quotSignReg, aInverter, quotIterReg))
  )
  quotM1Iter := Mux(
    state(s_iter),
    quotM1IterNext,
    Mux(state(s_pre_1), 0.U(len.W), Mux(quotSignReg, dInverter, quotM1IterReg))
  )
  ...
  val rFinal =
    RegEnable(
      Mux(specialReg, remSpecialReg, rShifted),
      state(s_post_1)
    ) // right shifted remainder. shift by the number of bits divisor is shifted
