...
class CustomGemmTest extends AnyFlatSpec with ChiselScalatestTester with ParallelTestExecution {
  ...
  private def testCustomGemm(dut: CustomGemm)(implicit config: DataWidthConfig) = {
    val p = dut.p
    val q = dut.q
    val m = dut.m

    val arraySize = 1
    val matrixAArray = Array.tabulate(arraySize)(_ => matInit[Float](p, m))
    val matrixBArray = Array.tabulate(arraySize)(_ => matInit[Float](m, q))
    val matrixArray = matrixAArray.zip(matrixBArray).map {
      case (a, b) => mmul(a, b)
    }
    printmat[Float]((matrixArray(0).flatten), p, q)
    def checkresult(): List[Float] = {
      val ret = for (i <- 0 until p * q) yield {
        val out = dut.io.out.bits(i).peekInt().floatValue
        print(f"${out.asInstanceOf[Float]}%.4f\t")
        if((i + 1)%p == 0){
          print("\n")
        }
    ...
    fork {
      ...
      while (c < arraySize) {
        if (dut.io.in_a.ready.peekBoolean() && dut.io.in_b.ready.peekBoolean()) {
          ...
          for (i <- 0 until m) {
            for (j <- 0 until p) {
              dut.io.in_a.bits(j * m + i).poke(toBinaryBigInt[Float](matrixAArray(c)(j)(i)).U)
            }
            for (j <- 0 until q) {
              dut.io.in_b.bits(i * q + j).poke(toBinaryBigInt[Float](matrixBArray(c)(i)(j)).U)
            }
          }
          c += 1
        } else {
  ...
  "CustomGemm basic test on Verilator" should "pass" in {
    ...
    .withAnnotations(Seq(VerilatorBackendAnnotation))(testCustomGemm)
  }
}
